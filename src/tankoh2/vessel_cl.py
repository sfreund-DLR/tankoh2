...
from tankoh2 import pychain, programDir
...
def winding_helical_layer(friction):
    print('--------------------')
    print('use friction', friction)
    vessel.setLayerFriction(layerindex, abs(friction), True)
    print('set friction', friction)
    ...
    try:
        ...
        print('I have to pass')

    # print('this helical layer shoud end at', wendekreisradius[layerindex], 'mm but is at', wk, 'mm so there is a deviation of', wendekreisradius[layerindex]-wk, 'mm')
    # if abs(wendekreisradius[layerindex]-wk) < 2.:
    #    arr_fric.append(abs(friction))
    #    arr_wk.append(wk)
...
def optimze_winding_parameters_friction():
    ...
    popt = minimize_scalar(winding_helical_layer, tol=0.00001, method='Golden', options={"maxiter": 1000, 'disp': True})
    # popt  = minimize(winding_helical_layer, x0 = (1.), method = 'BFGS', options={'gtol': 1e-6, 'disp': True})
...
def winding_hoop_layer(shift):
    vessel.setHoopLayerShift(layerindex, shift, True)
    vessel.runWindingSimulation(layerindex + 1)
    wk = vessel.getPolarOpeningR(layerindex, True)

    # print('this hoop layer shoud end at', krempenradius[layerindex], 'mm but is at', wk, 'mm so there is a deviation of', krempenradius[layerindex]-wk, 'mm')

    return abs(wk - krempenradius[layerindex])


def optimze_winding_parameters_shift():
    popt = minimize_scalar(winding_hoop_layer, tol=0.001)

    shift = popt.x

    return shift, winding_hoop_layer(shift)
...
filename = open(os.path.join(programDir, 'data', "Dome_contour_" + tankname + ".txt"), "r")
...
contour = open("Dome_contour_" + tankname + "_modified.dcon", "w")
...
print('Build Dome with dome data', dome)
...
liner.saveToFile(tankname + ".liner")
print('saved liner')
...
with open(tankname + ".liner", "w") as jsonFile:
...
material.loadFromFile(os.path.join(programDir, 'data', "CFRP_HyMod.json"))
mat = pychain.material

# read winding angles in cylindrical regime
filename = open("Winding_" + tankname + ".txt", "r")
Data = np.loadtxt(filename)
...
# rename design
with open(tankname + ".design") as jsonFile:
    data = json.load(jsonFile)
data["designs"]["1"]["name"] = tankname
with open(tankname + ".design", "w") as jsonFile:
    json.dump(data, jsonFile)
...
global layerindex
file = open(tankname + "_realised_winding.txt", "w")
file.write("Layer number" + '\t' + "Angle" + '\t' + "Polar opening" + '\n')
vessel.resetWindingSimulation()
for i in range(5):  # len(angle_degree)
    print('--------------------------------------------------')
    print('apply layer', i + 1, 'with angle', angle_degree[i])
    layerindex = i
    # wk = winding_layer(i, 0.5)
    if angle_degree[i] == 90.:
        print('Sollwendekreisradius', krempenradius[i])
        shift, err_wk = optimze_winding_parameters_shift()
        print('optimised shift is ', shift, 'resulting in a polar opening error of ', err_wk,
              'as current polar opening is', vessel.getPolarOpeningR(layerindex, True))
    else:
        # global arr_fric, arr_wk
        # arr_fric = []
        # arr_wk = []
        print('Sollwendekreisradius', wendekreisradius[i])
        friction, err_wk = optimze_winding_parameters_friction()
        print('optimised friction is ', friction, 'resulting in a polar opening error of ', err_wk,
              'as current polar opening is', vessel.getPolarOpeningR(layerindex, True))
    ...
    file.write(str(i + 1) + '\t' + str(angle_degree[i]) + '\t' + str(vessel.getPolarOpeningR(layerindex, True)) + '\n')

file.close()

# save vessel
vessel.saveToFile(tankname + ".vessel")  # save vessel

with open(tankname + ".vessel") as jsonFile:
    data = json.load(jsonFile)
data["vessel"]["name"] = tankname
with open(tankname + ".vessel", "w") as jsonFile:
    json.dump(data, jsonFile)
...
windingResults.saveToFile(tankname + ".wresults")
...
scriptGenerator.writeVesselAxSolidBuildScript(tankname + "_Build.py", settings, modelOptions)
scriptGenerator.writeVesselAxSolidBuildScript(tankname + "_Eval.py", settings, modelOptions)
